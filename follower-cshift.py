...
class Scan_msg:
	...
	def ros2cv(self, rosimg):
		return self.bridge.imgmsg_to_cv2(data, "bgr8")
	
	def processScan(self, laserscan):
	...
	def for_callback_laser(self,laserscan):
		...
		self.processScan(laserscan)
		self.movement()

	def for_callback_image(self,img):
		'''Passes laserscan onto function sort which gives the sect 
		variables the proper values.  Then the movement function is run 
		with the class sect variables as parameters.
...
def call_back_image(scanmsg):
	'''Passes laser scan message to for_callback function of sub_obj.
	Parameter scanmsg is laserscan message.'''
	sub_obj.for_callback_laser(image)
...
def listener():
	...
	scan = rospy.Subscriber('/scan', LaserScan, call_back_laser)
	img = rospy.Subscriber('/camera/rgb/image_raw', Image, call_back_image)
	joy = rospy.Subscriber('/joy', Joy, call_back_joy)
	rospy.spin()
